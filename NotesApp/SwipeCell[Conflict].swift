...
class SwipeCell: UITableViewCell {
  ...
  func handleSwipeGesture(gesture: UIPanGestureRecognizer) {
    ...
    if state == .Began {
      // began
      dragging = true
      let snapshot = swipeCreateScreenShot(self)
      swipeCreateView(snapshot: snapshot)
      ...
      swipeMoveCell(offset: CGRectGetMinX(contentScreenshotView.frame), direction: direction)
      swipeDelegate?.swipeTableViewCell(cell: self, didSwipeWithPercentage: percentage)
    } else if state == .Cancelled || state == .Ended {
      // ended or cancelled
      dragging = false
      isExiting = true
      swipeCompleteCell(percentage: percentage, duration: duration, direction: direction)
  ...
  // MARK: - BEGIN
  private func swipeCreateScreenShot(view: UIView) -> UIImage {
  ...
  private func swipeCreateView(snapshot snapshot: UIImage) {
  ...
  // MARK: - CHANGED
  private func swipeMoveCell(offset offset: CGFloat, direction: SwipeDirection) {
  ...
  private func swipeCompleteCell(percentage percentage: CGFloat, duration: Double, direction: SwipeDirection) {
    ...
    if let object = swipeGetSwipeObject(percentage: percentage) {
      ...
      if swipeGetBeforeTrigger(percentage: percentage, direction: direction) || mode == .Bounce {
        // bounce
        swipeDirectionBounce(duration: duration, direction: direction, icon: icon, percentage: percentage, completion: completion)
      } else {
        // slide
      ...
      // bounce
      swipeDirectionBounce(duration: duration, direction: direction, icon: nil, percentage: percentage, completion: nil)
    }
  }
  
  private func swipeDirectionBounce(duration duration: NSTimeInterval, direction: SwipeDirection, icon: UIView?, percentage: CGFloat, completion: SwipeCompletion?) {
  ...
  // MARK: - GET
  private func swipeGetSwipeObject(percentage percentage: CGFloat) -> SwipeObject? {
